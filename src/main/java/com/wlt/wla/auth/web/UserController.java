...
public class UserController {
	...
	public String removeItem(@ModelAttribute("Item") DBWishItems item) {
		...
		String sql = "UPDATE `dr_wishlist`.`wishlist_items` SET `status` = '-1' WHERE `wishlist_items`.`id` ="
				+ item.getId();
		jdbcTemp.execute(sql);
...
//		else System.out.println("else here");
	...
	public String fulfill(@ModelAttribute("Item") DBWishItems item) {
		...
		query = "SELECT name, price FROM dr_wishlist.wishlist_items WHERE user_id = " + userId + " AND id = "
				+ item.getId();
		...
		query = "INSERT INTO dr_wishlist.balance (user_id, balance_changes, note, timestamp) VALUES (" + userId + ", "
				+ (list.get(0).getPrice() * -1) + ", '" + note + "', '" + formatter.format(date) + "')";
		jdbcTemp.execute(query);
		query = "UPDATE dr_wishlist.wishlist_items SET status = 1 WHERE user_id = " + userId + " AND id = "
				+ item.getId();
		jdbcTemp.execute(query);
