...
<%-- 							<form:form name="SavePwd" id="SavePwd" method="POST" --%>
<%-- 								name="SavePwd" id="SaveGame" action="${contextPath}/setPwd" --%>
<%-- 								items="${UlistEmp}"> --%>
<%-- 								<input type="hidden" name="id" value="${emp.id}" /> --%>
...
<!-- SHOULD BE BETTER WAY TO DO THAT INSTEAD OF BUNCH save funcs -->
        <script>
            function save${emp.id}() {
                $("#password${emp.id}").val(prompt("Enter new password:"))
            }
        </script>
